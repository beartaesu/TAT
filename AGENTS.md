...
- 회사 전달용 작업은 `handoff/TAT_현재작업.html` 저장을 곧 MYBOX 업로드 대상으로 간주한다. 이 폴더는 이미 MYBOX 탐색기 1:1 동기화가 설정·검증되어 있으므로, 새 작업마다 MYBOX 웹 로그인이나 수동 업로드를 다시 요구하지 않는다. 파일 갱신 후에는 로컬 저장 완료와 동기화 대상 경로를 사용자에게 명시하고, 동기화 실패가 실제로 확인된 경우에만 별도 복구 절차를 진행한다.
