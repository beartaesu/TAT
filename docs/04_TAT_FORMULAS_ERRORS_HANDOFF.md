...
위임 경고 후속안(저장소 반영 완료, Power Apps 검증 대기):

- SharePoint `접수항목상세`와 `접수시료항목매핑`에 숫자 열 `접수헤더ID복사`를 생성하고 인덱스를 설정했다.
- 신규 상세·매핑 저장 7곳에서 `Round(Value(var저장헤더.ID), 0)`을 기록한다.
- 기존 상세·매핑 삭제 6곳은 `접수헤더ID복사 = Round(Value(var저장헤더.ID), 0)` 숫자 동등 비교를 사용한다.
- GitHub Raw 전체 수식을 Power Apps에 적용한 뒤 문법, 위임 경고, 신규·수정 저장의 삭제·재생성을 실제 검증한다.
...
- 현재 저장소 기준 SHA-256: `2645f22bb3654e34c12d9b2168bda079fa9badb289bf859758f0cd0630103320`
- 전체 줄 수: 1,393줄
- 직접 `접수헤더연결: { ... }` 레코드: 7곳
- `접수헤더ID복사` 저장 속성: 7곳
- `접수헤더ID복사` 숫자 `RemoveIf` 조건: 6곳
- `접수헤더연결.Id = var저장헤더.ID`: 0곳
...
## 9. 2026-08-13 `접수헤더ID복사` 적용 인계

- 정확한 위치: `scr검토저장 > btn최종저장 > OnSelect`.
- SharePoint 준비 상태: `접수항목상세`, `접수시료항목매핑`의 숫자 열 및 인덱스 생성 완료.
- 수식 상태: `접수헤더ID복사` 저장 7곳, 숫자 `RemoveIf` 조건 6곳, 직접 `접수헤더연결` 레코드 7곳.
- 기존 `접수헤더연결.Id = var저장헤더.ID` 조건은 남아 있지 않다.
- Power Apps 검증 대기: 전체 수식 교체 → 문법·위임 경고 확인 → 신규 저장 → 접수내용 수정 저장 → 상세·매핑 중복 및 연결 확인.
