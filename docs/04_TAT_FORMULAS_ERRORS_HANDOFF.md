...
## 7. 최종저장 OnSelect 15개 인수 교체본 — 2026-08-05

### 위치

`scr검토저장 > btn최종저장 > OnSelect`

### 확인 결과

- Power Apps (V2) 입력 순서: 접수헤더ID, 접수번호, 의뢰구분, 현장명, 과제명, 시료수, 시료명목록, 분석항목목록, 시료항목매핑, 처리모드, 변경전의뢰구분, 변경전접수번호, 변경전결과폴더경로, 변경전결과엑셀경로, 변경이력ID
- 숫자 입력: 1번 접수헤더ID, 6번 시료수, 15번 변경이력ID
- 텍스트 입력: 나머지 12개
- 실제 첨부 수식의 Flow 호출부: 총 4곳
- 완료 건 수정 1곳: 처리모드 `수정`
- 신규 저장 분기 3곳: 처리모드 `신규`
- 네 호출 모두 11~14번 빈 문자열, 15번 숫자 `0` 추가
- 전체 수식 기준본: [formulas/scr검토저장_btn최종저장_OnSelect.powerfx](../formulas/scr검토저장_btn최종저장_OnSelect.powerfx)

### 오류 상태

기존 `15개의 인수가 필요한데 10개를 받았습니다` 오류의 수식 수정안을 Power Apps에 적용했다. 2026-08-05 사용자 확인 결과 인수 오류와 빨간 문법 오류가 모두 사라졌다. 아직 실제 앱 저장 및 새 접수 Flow 실행 성공 여부는 미검증 상태다.

## 8. 현재 재개 위치 — 2026-08-05
...
1. Power Apps 최종저장 성공·실패 알림
2. `TAT_결과파일생성_App`의 새 실행 기록 생성 여부
3. 결과 폴더명과 실제 폴더 생성 여부
4. 결과 Excel 파일명과 실제 파일 생성 여부
5. 접수헤더의 결과폴더명·경로·결과엑셀파일명·경로 및 폴더처리상태
6. Flow가 멈추거나 실패하면 최초 빨간 작업의 입력·출력 확인
